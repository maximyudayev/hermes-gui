...
class NotesStreamer(Producer):
    """A streamer that allows the user to enter timestamped notes during an experiment."""
            ...
            self.add_stream(
                ...
                data_notes=OrderedDict(
                    [
                        (
                            "Description",
                            "Notes that the experimenter entered "
    ...
    def get_last_notes(self):
        ...
        data = self.get_data(
            device_name, stream_name, starting_index=-1, ending_index=None
        )
        if data is not None:
            return data["data"][0]
        return data
    ...
    """
        ...
        try:
            while self._running:
                ...
                if len(notes) > 0:
                    self.append_data(
                        self.get_device_names()[0], "notes", notes_times_s, notes
                    )
